...
def build_aegis_mixer_decision(
    *,
    ...
    for candidate in shortlist:
        candidate.ranking_score = _rank_candidate(
            candidate=candidate,
            cell=cell,
            budget=budget,
            controller_queue=controller_queue,
            total_requested_prbs=total_requested_prbs,
            total_requested_gpu=total_requested_gpu,
            overload=overload,
            attack_pressure=attack_pressure,
        )
    ...
    for slice_demand in cell.slices:
        ...
        decisions[slice_demand.slice_id] = SliceDecision(
            ...
            allocated_prbs=fallback.allocated_prbs,
            allocated_gpu=fallback.allocated_gpu,
...
def _rank_candidate(
    *,
    ...
    budget: ResourceBudget,
    controller_queue: int,
    total_requested_prbs: float,
    total_requested_gpu: float,
    overload: bool,
    attack_pressure: float,
) -> float:
    ...
    budget_pressure = (total_requested_prbs / max(budget.prbs_per_cell, 1e-6)) + (total_requested_gpu / max(budget.gpu_per_cell, 1e-6))
    ...
    attack_bonus = 3.4 * slice_demand.anomaly_score if candidate.isolated else 1.2 * slice_demand.anomaly_score if candidate.inspected else -0.5 * slice_demand.anomaly_score
    ...
    ai_floor_bonus = 1.3 if overload and slice_demand.kind == "ai_edge" and candidate.action_name in {"serve_degraded", "serve_floor"} else 0.0
    mission_floor_bonus = 1.2 if overload and slice_demand.mission_critical and candidate.action_name in {"protect_full", "protect_floor"} else 0.0
...
def _fallback_candidate(
    *,
    ...
    prb_ratio = 0.88 if slice_demand.mission_critical else 0.78 if slice_demand.kind == "ai_edge" else 0.45 if slice_demand.kind == "broadband" else 0.25
    ...
    *,
    ...
    for candidate in ranked_candidates:
        ...
        if not candidate.isolated and (not fits_prb or not fits_gpu):
            continue
        decisions[candidate.slice_id] = SliceDecision(
